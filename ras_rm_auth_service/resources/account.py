...
def post_account():
    ...
    try:
        ...
        return make_response(
            jsonify({"title": "Authentication error in Auth service", "detail": "Missing 'username' or 'password'"}),
            400,
        )
...
def put_account():
    ...
    try:
        ...
        logger.info("Missing request parameter", exc_info=ex)
        return make_response(
            jsonify({"title": "Auth service account update user error", "detail": "Missing 'username'"}), 400
        )
    ...
    try:
        ...
        return make_response(
            jsonify(
                {"title": "Auth service account update user error", "detail": "account_verified status is invalid"}
            ),
            400,
        )
    except SQLAlchemyError:
...
def patch_account(username):
    ...
    try:
        ...
        return make_response(
            jsonify({"title": "Bad Request error in Auth service", "detail": "Patch data validation failed"}), 400
        )
    try:
        with transactional_session() as session:
            user = session.query(User).filter(func.lower(User.username) == username.lower()).one()
...
def delete_account():
    ...
    try:
        ...
        return make_response(jsonify({"title": "Auth service delete user error", "detail": "Missing 'username'"}), 400)
    except NoResultFound:
        logger.error("User does not exist", username=obfuscate_email(username))
        return make_response(
            jsonify(
        ...
        return make_response(
            jsonify({"title": "Auth service delete user error", "detail": "Unable to commit delete operation"}), 500
        )
