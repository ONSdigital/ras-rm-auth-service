...
def process_accounts_for_first_notification():
    """
    Sends Notification to the user accounts which has not been accessed in the last 24 months
    """
    ...
    for username in users:
        logger.info(f"Sending first due deletion notification {username}")
        ...
        con.commit()
        logger.info(f"Due deletion first notification sent to the {username}")
    csr.close()
    con.close()
    logger.info("Scheduler finished processing Accounts not accessed in the last 24 months ")
