...
class TestDueDeletionSecondNotification(unittest.TestCase):

    def test_successful_notification(self):
        with patch('ras_rm_auth_scheduler_service.scheduled_jobs.due_deletion_second_notification.setup')as setup:
            with patch('ras_rm_auth_scheduler_service.scheduled_jobs.due_deletion_second_notification.NotifyService'
                       '.request_to_notify'
                       )as mock_notify:
